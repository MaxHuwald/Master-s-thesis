setwd("/project/sysviro/users/Max/analyses/Conditions/Results/Modifications")
library(dplyr)
library(readr)
library(stringr)
library(purrr)
...
plot_mod_distribution_hist <- function(df, folder, alignment, cond, code) {
  ...
  # --- NEW: title expression using data-driven plotmath string
  code_pm_str <- df_cond$code_pm[1]              # e.g. "m^6*A"
  code_pm_expr <- parse(text = code_pm_str)[[1]] # turn into an expression
  title_expr <- bquote(.(folder) ~ .(alignment) ~ .(cond) ~ .(code_pm_expr))
  
  ggplot(df_cond, aes(x = bin_center, y = frac, fill = condition)) +
    geom_col(width = bin_width, position = "dodge", color = "blue") +
    ...
    labs(
      title = title_expr,                      # <-- expression title
...
# Generate individual plots for Total HSV2
plots_hsv2 <- purrr::map(
  condition_levels[condition_levels != "4h"],
...
# Generate individual plots for Total hg38
plots_hg38 <- purrr::map(
  condition_levels[condition_levels != "4h"],
  ~ plot_mod_distribution_hist(
    all_data %>% filter(condition != "4h"),
    folder   = "Total",
...
# Generate individual plots for eU HSV2
...
# Generate individual plots for eU hg38
...
# Generate individual plots for non-eU HSV2
...
# Generate individual plots for non-eU hg38
...
combined_hsv2 <- wrap_plots(plots_hsv2, ncol = 2)
combined_hg38 <- wrap_plots(plots_hg38, ncol = 2)
...
ggsave(paste0("combined_hsv2_", mod_code, ".png"), 
       plot = combined_hsv2, width = 6, height = 8, dpi = 300, device = "png", units = "in")

ggsave(paste0("combined_hg38_", mod_code, ".png"), 
       plot = combined_hg38, width = 6, height = 8, dpi = 300, device = "png", units = "in")

ggsave(paste0("combined_hsv2_eU_", mod_code, ".png"), 
       plot = combined_hsv2_eU, width = 7, height = 8, dpi = 300, device = "png", units = "in")

ggsave(paste0("combined_hg38_eU_", mod_code, ".png"), 
       plot = combined_hg38_eU, width = 7, height = 8, dpi = 300, device = "png", units = "in")

ggsave(paste0("combined_hsv2_non_eU_", mod_code, ".png"), 
       plot = combined_hsv2_non_eU, width = 6, height = 8, dpi = 300, device = "png", units = "in")

ggsave(paste0("combined_hg38_non_eU_", mod_code, ".png"), 
       plot = combined_hg38_non_eU, width = 6, height = 8, dpi = 300, device = "png", units = "in")


### interesting note: modification calling probabilities seem to be cut for A and U
#subset_df <- all_data[all_data$bin_center > 0.94, ]
